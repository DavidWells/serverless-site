...
layout: Doc
gitLink: /docs/providers/aws/cli-reference/metrics.md
---
...
- `--startTime` A specific unit in time to start fetching metrics from (ie: `2020`, `1469705761`, `30m` (30 minutes ago), `2h` (2 days ago) or `3d` (3 days ago)). Date formats should be written in ISO 8601. Defaults to 24h ago.
- `--endTime` A specific unit in time to end fetching metrics from (ie: `2021` or `1469705761`). Date formats should be written in ISO 8601. Defaults to now.
...
```bash
serverless metrics --startTime 2001 --endTime 2002
```
...
```bash
serverless metrics --function hello --startTime 2001 --endTime 2002
```
